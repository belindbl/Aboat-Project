...
class UDPClient(object):
    ...
    def readSimple(self):
        ...
        try:
            while True:
                ...
                if len(data) < 12:
                    print("error recieving UDP packet: not enough byte for ALS Protocol")
                    break
                (messageID,packetID, message_size) = struct.unpack('<III', data[0:12])
                if packetID != 0:
                    print("not starting with first packet of the message, discarding (was ", messageID," ",packetID,")")
                    continue
                message = data[12:]
                while len(message) != message_size:
                    ...
                    if messageID != newMessageID or packetID+1 != newPacketID:
                        print("messages are not ordered: discarding: ",newPacketID,"(",packetID,") mesID ", messageID,"(", newMessageID,")")
                        message = ""
                        break
    ...
    def _reraise_exception_as_UDP_error(self, message, exception):
        #pass
        raise UDPConnectionError('%s:%s - %s: %s' % (self._host, self._port, message, exception))
