import sys, ctypes, struct, threading, socket, re, time, logging, atexit
try:
...
class Client(object):
	...
	def __raw_message_handler(self, raw_message):
		...
		if match:
			...
			if message_id != self.message_id:
				if message_id not in self.timeouted_requests:
					raise Exception(f"Message ID mismatch: received {message_id} =/= ccurrent {self.message_id} (local)")
				self.timeouted_requests.remove(message_id)	
			self.response = message_body
			if not self.waiting_for_async_message:
				self.wait_response.set()
		else:
	...
	def request(self, message, timeout=5):
		...
		#Append the timeouted request to the list
		self.timeouted_requests.append(self.message_id)
		self.message_id += 1
		self.wait_response.clear()
		self.lock.release()
		...
		if isset:
			...
			_L.error('Can not receive a response from server, timeouted after %.2f seconds', timeout)
			return None
	...
	def execute(self, message, timeout=5):
		...
		def do_exe():
			raw_message = b'%d:%s' % (-self.message_id, message)
			_L.debug('Execute: %s', raw_message.decode("utf-8"))
			if not self.message_client.send(raw_message):
				return None
	...
	def request_open_level(self, levelname:str, timeout:float=5):
		...
		if not (isinstance(levelname, str)):
			print("open_level: levelname has wrong type. Expected str got {}".format(levelname.__class__.__name__))
		return_value = self.request("OpenLevel {}".format(levelname), timeout)
		self.wait_for_task_complete()
		return return_value

	def execute_teleport(self, posx:float, posy:float, posz:float, timeout:float=5):
		"""Teleport all EgoVehicles to one position. \n\tUsage: Teleport [posx] [posy] [posz]"""
		if not (isinstance(posx, int) or isinstance(posx, float)):
			print("teleport: posx has wrong type. Expected float got {}".format(posx.__class__.__name__))
		if not (isinstance(posy, int) or isinstance(posy, float)):
			print("teleport: posy has wrong type. Expected float got {}".format(posy.__class__.__name__))
		if not (isinstance(posz, int) or isinstance(posz, float)):
			print("teleport: posz has wrong type. Expected float got {}".format(posz.__class__.__name__))
		self.execute("Teleport {} {} {}".format(posx, posy, posz), timeout)

	def request_teleport(self, posx:float, posy:float, posz:float, timeout:float=5):
		"""Teleport all EgoVehicles to one position. \n\tUsage: Teleport [posx] [posy] [posz]"""
		if not (isinstance(posx, int) or isinstance(posx, float)):
			print("teleport: posx has wrong type. Expected float got {}".format(posx.__class__.__name__))
		if not (isinstance(posy, int) or isinstance(posy, float)):
	...
	def execute_rotate(self, pitch:float, roll:float, yaw:float, timeout:float=5):
		...
		if not (isinstance(pitch, int) or isinstance(pitch, float)):
			print("rotate: pitch has wrong type. Expected float got {}".format(pitch.__class__.__name__))
		if not (isinstance(roll, int) or isinstance(roll, float)):
			print("rotate: roll has wrong type. Expected float got {}".format(roll.__class__.__name__))
		if not (isinstance(yaw, int) or isinstance(yaw, float)):
			print("rotate: yaw has wrong type. Expected float got {}".format(yaw.__class__.__name__))
	...
	def request_load_scenario(self, scenarioname:str, timeout:float=5):
		...
		if not (isinstance(scenarioname, str)):
			print("load_scenario: scenarioname has wrong type. Expected str got {}".format(scenarioname.__class__.__name__))
		return_value = self.request("LoadScenario {}".format(scenarioname), timeout)
		self.wait_for_task_complete()
		return return_value

	def request_load_scenario_from_string(self, scenariostring:str, timeout:float=5):
		"""Load a scenario from a string. \n\tUsage: LoadScenarioFromString [scenariostring]"""
		if not (isinstance(scenariostring, str)):
			print("load_scenario_from_string: scenariostring has wrong type. Expected str got {}".format(scenariostring.__class__.__name__))
		return_value = self.request("LoadScenarioFromString {}".format(scenariostring), timeout)
		self.wait_for_task_complete()
		return return_value
	...
	def request_load_sub_scene(self, name:str, load:bool, timeout:float=5):
		...
		if not (isinstance(name, str)):
			print("load_sub_scene: name has wrong type. Expected str got {}".format(name.__class__.__name__))
		if not (isinstance(load, bool)):
			print("load_sub_scene: load has wrong type. Expected bool got {}".format(load.__class__.__name__))
	...
	def execute_load_situation(self, situationfilename:str, timeout:float=5):
		...
		if not (isinstance(situationfilename, str)):
			print("load_situation: situationfilename has wrong type. Expected str got {}".format(situationfilename.__class__.__name__))
	...
	def request_load_situation_layer(self, situation:str, timeout:float=5):
		...
		if not (isinstance(situation, str)):
			print("load_situation_layer: situation has wrong type. Expected str got {}".format(situation.__class__.__name__))
	...
	def execute_load_weather(self, weatherfilename:str, timeout:float=5):
		...
		if not (isinstance(weatherfilename, str)):
			print("load_weather: weatherfilename has wrong type. Expected str got {}".format(weatherfilename.__class__.__name__))
	...
	def execute_load_sea_state(self, seastatefilename:str, timeout:float=5):
		...
		if not (isinstance(seastatefilename, str)):
			print("load_sea_state: seastatefilename has wrong type. Expected str got {}".format(seastatefilename.__class__.__name__))
	...
	def execute_set_parameter_int(self, name:str, value:int, timeout:float=5):
		...
		if not (isinstance(name, str)):
			print("set_parameter_int: name has wrong type. Expected str got {}".format(name.__class__.__name__))
		if not (isinstance(value, int)):
			print("set_parameter_int: value has wrong type. Expected int got {}".format(value.__class__.__name__))
	...
	def execute_set_parameter_float(self, name:str, value:float, timeout:float=5):
		...
		if not (isinstance(name, str)):
			print("set_parameter_float: name has wrong type. Expected str got {}".format(name.__class__.__name__))
		if not (isinstance(value, int) or isinstance(value, float)):
			print("set_parameter_float: value has wrong type. Expected float got {}".format(value.__class__.__name__))
	...
	def execute_set_simulation_speed_factor(self, value:float, timeout:float=5):
		...
		if not (isinstance(value, int) or isinstance(value, float)):
			print("set_simulation_speed_factor: value has wrong type. Expected float got {}".format(value.__class__.__name__))
	...
	def execute_start_seed_override(self, seed:int, timeout:float=5):
		...
		if not (isinstance(seed, int)):
			print("start_seed_override: seed has wrong type. Expected int got {}".format(seed.__class__.__name__))
	...
	def execute_spawn_debug_sphere(self, posx:float, posy:float, posz:float, scalex:float, scaley:float, scalez:float, sphere_color:str, duration_in_seconds:float, timeout:float=5):
		...
		if not (isinstance(posx, int) or isinstance(posx, float)):
			print("spawn_debug_sphere: posx has wrong type. Expected float got {}".format(posx.__class__.__name__))
		if not (isinstance(posy, int) or isinstance(posy, float)):
			print("spawn_debug_sphere: posy has wrong type. Expected float got {}".format(posy.__class__.__name__))
		if not (isinstance(posz, int) or isinstance(posz, float)):
			print("spawn_debug_sphere: posz has wrong type. Expected float got {}".format(posz.__class__.__name__))
		if not (isinstance(scalex, int) or isinstance(scalex, float)):
			print("spawn_debug_sphere: scalex has wrong type. Expected float got {}".format(scalex.__class__.__name__))
		if not (isinstance(scaley, int) or isinstance(scaley, float)):
			print("spawn_debug_sphere: scaley has wrong type. Expected float got {}".format(scaley.__class__.__name__))
		if not (isinstance(scalez, int) or isinstance(scalez, float)):
			print("spawn_debug_sphere: scalez has wrong type. Expected float got {}".format(scalez.__class__.__name__))
		if not (isinstance(sphere_color, str)):
			print("spawn_debug_sphere: sphere_color has wrong type. Expected str got {}".format(sphere_color.__class__.__name__))
		if not (isinstance(duration_in_seconds, int) or isinstance(duration_in_seconds, float)):
			print("spawn_debug_sphere: duration_in_seconds has wrong type. Expected float got {}".format(duration_in_seconds.__class__.__name__))
		self.execute("SpawnDebugSphere {} {} {} {} {} {} {} {}".format(posx, posy, posz, scalex, scaley, scalez, sphere_color, duration_in_seconds), timeout)

	def request_spawn_debug_sphere(self, posx:float, posy:float, posz:float, scalex:float, scaley:float, scalez:float, sphere_color:str, duration_in_seconds:float, timeout:float=5):
		"""Spawns a debugging sphere. \n\tUsage: SpawnDebugSphere [posx] [posy] [posz] [scalex] [scaley] [scalez] [redphere/greenphere/bluephere] [duration in seconds]"""
		if not (isinstance(posx, int) or isinstance(posx, float)):
			print("spawn_debug_sphere: posx has wrong type. Expected float got {}".format(posx.__class__.__name__))
		if not (isinstance(posy, int) or isinstance(posy, float)):
		...
		if not (isinstance(duration_in_seconds, int) or isinstance(duration_in_seconds, float)):
			print("spawn_debug_sphere: duration_in_seconds has wrong type. Expected float got {}".format(duration_in_seconds.__class__.__name__))
		return self.request("SpawnDebugSphere {} {} {} {} {} {} {} {}".format(posx, posy, posz, scalex, scaley, scalez, sphere_color, duration_in_seconds), timeout)
	...
	def execute_set_cloud_style(self, style:str, timeout:float=5):
		...
		if not (isinstance(style, str)):
			print("set_cloud_style: style has wrong type. Expected str got {}".format(style.__class__.__name__))
	...
	def execute_set_time_of_day(self, time_ISO_8601_string:str, timeout:float=5):
		...
		if not (isinstance(time_ISO_8601_string, str)):
			print("set_time_of_day: time_ISO_8601_string has wrong type. Expected str got {}".format(time_ISO_8601_string.__class__.__name__))
	...
	def execute_set_cloud_coverage(self, cloud_style:float, timeout:float=5):
		...
		if not (isinstance(cloud_style, int) or isinstance(cloud_style, float)):
			print("set_cloud_coverage: cloud_style has wrong type. Expected float got {}".format(cloud_style.__class__.__name__))
	...
	def execute_set_season(self, season:str, timeout:float=5):
		...
		if not (isinstance(season, str)):
			print("set_season: season has wrong type. Expected str got {}".format(season.__class__.__name__))
	...
	def execute_set_ground_wetness_type(self, type:str, timeout:float=5):
		...
		if not (isinstance(type, str)):
			print("set_ground_wetness_type: type has wrong type. Expected str got {}".format(type.__class__.__name__))
	...
	def execute_set_ground_wetness_percent(self, percent:float, timeout:float=5):
		...
		if not (isinstance(percent, int) or isinstance(percent, float)):
			print("set_ground_wetness_percent: percent has wrong type. Expected float got {}".format(percent.__class__.__name__))
	...
	def execute_set_rain_type(self, type:str, timeout:float=5):
		...
		if not (isinstance(type, str)):
			print("set_rain_type: type has wrong type. Expected str got {}".format(type.__class__.__name__))
	...
	def execute_set_rain_intensity(self, intensity:float, timeout:float=5):
		...
		if not (isinstance(intensity, int) or isinstance(intensity, float)):
			print("set_rain_intensity: intensity has wrong type. Expected float got {}".format(intensity.__class__.__name__))
	...
	def execute_replay(self, filename:str, timeout:float=5):
		...
		if not (isinstance(filename, str)):
			print("replay: filename has wrong type. Expected str got {}".format(filename.__class__.__name__))
	...
	def execute_load_situation_with_overrides(self, situation:str, override:str, timeout:float=5):
		...
		if not (isinstance(situation, str)):
			print("load_situation_with_overrides: situation has wrong type. Expected str got {}".format(situation.__class__.__name__))
		if not (isinstance(override, str)):
			print("load_situation_with_overrides: override has wrong type. Expected str got {}".format(override.__class__.__name__))
		self.execute("LoadSituationWithOverrides {} {}".format(situation, override), timeout)

	def request_load_situation_with_overrides(self, situation:str, override:str, timeout:float=5):
		"""Load and override variables for one of the situation files \n\tUsage: LoadSituationWithOverrides [situation] [overridefile/overridestring]"""
		if not (isinstance(situation, str)):
			print("load_situation_with_overrides: situation has wrong type. Expected str got {}".format(situation.__class__.__name__))
		if not (isinstance(override, str)):
			print("load_situation_with_overrides: override has wrong type. Expected str got {}".format(override.__class__.__name__))
		return self.request("LoadSituationWithOverrides {} {}".format(situation, override), timeout)

	def request_load_situation_layer_with_overrides(self, situation:str, override:str, timeout:float=5):
		...
		if not (isinstance(situation, str)):
			print("load_situation_layer_with_overrides: situation has wrong type. Expected str got {}".format(situation.__class__.__name__))
		if not (isinstance(override, str)):
			print("load_situation_layer_with_overrides: override has wrong type. Expected str got {}".format(override.__class__.__name__))
		return_value = self.request("LoadSituationLayerWithOverrides {} {}".format(situation, override), timeout)
		self.wait_for_task_complete()
		return return_value

	def execute_load_weather_with_overrides(self, weather_file_name:str, override:str, timeout:float=5):
		...
		if not (isinstance(weather_file_name, str)):
			print("load_weather_with_overrides: weather_file_name has wrong type. Expected str got {}".format(weather_file_name.__class__.__name__))
		if not (isinstance(override, str)):
			print("load_weather_with_overrides: override has wrong type. Expected str got {}".format(override.__class__.__name__))
		self.execute("LoadWeatherWithOverrides {} {}".format(weather_file_name, override), timeout)

	def request_load_weather_with_overrides(self, weather_file_name:str, override:str, timeout:float=5):
		"""Load and override variables for one of the weather files if supported by the scene. \n\tUsage: LoadWeatherWithOverrides [weather file name] [path to override file/override file content string]"""
		if not (isinstance(weather_file_name, str)):
			print("load_weather_with_overrides: weather_file_name has wrong type. Expected str got {}".format(weather_file_name.__class__.__name__))
		if not (isinstance(override, str)):
			print("load_weather_with_overrides: override has wrong type. Expected str got {}".format(override.__class__.__name__))
		return self.request("LoadWeatherWithOverrides {} {}".format(weather_file_name, override), timeout)

	def execute_load_sea_state_with_overrides(self, sea_state_file_name:str, override:str, timeout:float=5):
		...
		if not (isinstance(sea_state_file_name, str)):
			print("load_sea_state_with_overrides: sea_state_file_name has wrong type. Expected str got {}".format(sea_state_file_name.__class__.__name__))
		if not (isinstance(override, str)):
			print("load_sea_state_with_overrides: override has wrong type. Expected str got {}".format(override.__class__.__name__))
		self.execute("LoadSeaStateWithOverrides {} {}".format(sea_state_file_name, override), timeout)

	def request_load_sea_state_with_overrides(self, sea_state_file_name:str, override:str, timeout:float=5):
		"""Load and override variables for one of the sea state files if supported by the scene. \n\tUsage: LoadSeaStateWithOverrides [sea state file name] [path to override file/override file content string]"""
		if not (isinstance(sea_state_file_name, str)):
			print("load_sea_state_with_overrides: sea_state_file_name has wrong type. Expected str got {}".format(sea_state_file_name.__class__.__name__))
		if not (isinstance(override, str)):
			print("load_sea_state_with_overrides: override has wrong type. Expected str got {}".format(override.__class__.__name__))
		return self.request("LoadSeaStateWithOverrides {} {}".format(sea_state_file_name, override), timeout)

	def request_load_scenario_with_overrides(self, scenarioname:str, situation_overridefile:str, timeout:float=5):
		"""Load and override variables for a scenario. \n\tUsage: LoadScenarioWithOverrides [scenarioname] [situationoverridefile]"""
		if not (isinstance(scenarioname, str)):
			print("load_scenario_with_overrides: scenarioname has wrong type. Expected str got {}".format(scenarioname.__class__.__name__))
		if not (isinstance(situation_overridefile, str)):
			print("load_scenario_with_overrides: situation_overridefile has wrong type. Expected str got {}".format(situation_overridefile.__class__.__name__))
		return_value = self.request("LoadScenarioWithOverrides {} {}".format(scenarioname, situation_overridefile), timeout)
		self.wait_for_task_complete()
		return return_value
	...
	def execute_teleport_object(self, object:str, newx:float, newy:float, newz:float, filename:str, timeout:float=5):
		"""Teleport specified object to specified location. \n\tUsage: TeleportObject [path/alias/objectname] [newx] [newy] [newz] [filename]"""
		if not (isinstance(object, str)):
			print("teleport_object: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(newx, int) or isinstance(newx, float)):
			print("teleport_object: newx has wrong type. Expected float got {}".format(newx.__class__.__name__))
		if not (isinstance(newy, int) or isinstance(newy, float)):
			print("teleport_object: newy has wrong type. Expected float got {}".format(newy.__class__.__name__))
		if not (isinstance(newz, int) or isinstance(newz, float)):
			print("teleport_object: newz has wrong type. Expected float got {}".format(newz.__class__.__name__))
		if not (isinstance(filename, str)):
			print("teleport_object: filename has wrong type. Expected str got {}".format(filename.__class__.__name__))
		self.execute("TeleportObject {} {} {} {} {}".format(object, newx, newy, newz, filename), timeout)

	def request_teleport_object(self, object:str, newx:float, newy:float, newz:float, filename:str, timeout:float=5):
		"""Teleport specified object to specified location. \n\tUsage: TeleportObject [path/alias/objectname] [newx] [newy] [newz] [filename]"""
		if not (isinstance(object, str)):
			print("teleport_object: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(newx, int) or isinstance(newx, float)):
		...
		if not (isinstance(filename, str)):
			print("teleport_object: filename has wrong type. Expected str got {}".format(filename.__class__.__name__))
		return self.request("TeleportObject {} {} {} {} {}".format(object, newx, newy, newz, filename), timeout)

	def execute_teleport_object_gps(self, object:str, newlatitude:float, newlongitude:float, filename:str, timeout:float=5):
		...
		if not (isinstance(object, str)):
			print("teleport_object_gps: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(newlatitude, int) or isinstance(newlatitude, float)):
			print("teleport_object_gps: newlatitude has wrong type. Expected float got {}".format(newlatitude.__class__.__name__))
		if not (isinstance(newlongitude, int) or isinstance(newlongitude, float)):
			print("teleport_object_gps: newlongitude has wrong type. Expected float got {}".format(newlongitude.__class__.__name__))
		if not (isinstance(filename, str)):
			print("teleport_object_gps: filename has wrong type. Expected str got {}".format(filename.__class__.__name__))
		self.execute("TeleportObjectGPS {} {} {} {}".format(object, newlatitude, newlongitude, filename), timeout)

	def request_teleport_object_gps(self, object:str, newlatitude:float, newlongitude:float, filename:str, timeout:float=5):
		"""Teleport specified object to specified GPS coordinates. \n\tUsage: TeleportObjectGPS [path/alias/objectname] [newlatitude] [newlongitude] [filename]"""
		if not (isinstance(object, str)):
			print("teleport_object_gps: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(newlatitude, int) or isinstance(newlatitude, float)):
		...
		if not (isinstance(filename, str)):
			print("teleport_object_gps: filename has wrong type. Expected str got {}".format(filename.__class__.__name__))
		return self.request("TeleportObjectGPS {} {} {} {}".format(object, newlatitude, newlongitude, filename), timeout)

	def execute_rotate_object(self, object:str, newpitch:float, newyaw:float, newroll:float, filename:str, timeout:float=5):
		"""Rotate specified object to specified rotation. \n\tUsage: RotateObject [path/alias/objectname] [newpitch] [newyaw] [newroll] [filename]"""
		if not (isinstance(object, str)):
			print("rotate_object: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(newpitch, int) or isinstance(newpitch, float)):
			print("rotate_object: newpitch has wrong type. Expected float got {}".format(newpitch.__class__.__name__))
		if not (isinstance(newyaw, int) or isinstance(newyaw, float)):
			print("rotate_object: newyaw has wrong type. Expected float got {}".format(newyaw.__class__.__name__))
		if not (isinstance(newroll, int) or isinstance(newroll, float)):
			print("rotate_object: newroll has wrong type. Expected float got {}".format(newroll.__class__.__name__))
		if not (isinstance(filename, str)):
			print("rotate_object: filename has wrong type. Expected str got {}".format(filename.__class__.__name__))
		self.execute("RotateObject {} {} {} {} {}".format(object, newpitch, newyaw, newroll, filename), timeout)

	def request_rotate_object(self, object:str, newpitch:float, newyaw:float, newroll:float, filename:str, timeout:float=5):
		"""Rotate specified object to specified rotation. \n\tUsage: RotateObject [path/alias/objectname] [newpitch] [newyaw] [newroll] [filename]"""
		if not (isinstance(object, str)):
			print("rotate_object: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(newpitch, int) or isinstance(newpitch, float)):
		...
		if not (isinstance(filename, str)):
			print("rotate_object: filename has wrong type. Expected str got {}".format(filename.__class__.__name__))
		return self.request("RotateObject {} {} {} {} {}".format(object, newpitch, newyaw, newroll, filename), timeout)
	...
	def execute_set_physics_active(self, object:str, active:bool, timeout:float=5):
		...
		if not (isinstance(object, str)):
			print("set_physics_active: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(active, bool)):
			print("set_physics_active: active has wrong type. Expected bool got {}".format(active.__class__.__name__))
	...
	def execute_set_object_visibility(self, object:str, visible:bool, timeout:float=5):
		"""Set object visibility. \n\tUsage: SetObjectVisibility [path/alias/objectname] [true/false]"""
		if not (isinstance(object, str)):
			print("set_object_visibility: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(visible, bool)):
			print("set_object_visibility: visible has wrong type. Expected bool got {}".format(visible.__class__.__name__))
		self.execute("SetObjectVisibility {} {}".format(object, visible), timeout)

	def request_set_object_visibility(self, object:str, visible:bool, timeout:float=5):
		"""Set object visibility. \n\tUsage: SetObjectVisibility [path/alias/objectname] [true/false]"""
		if not (isinstance(object, str)):
			print("set_object_visibility: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(visible, bool)):
			print("set_object_visibility: visible has wrong type. Expected bool got {}".format(visible.__class__.__name__))
		return self.request("SetObjectVisibility {} {}".format(object, visible), timeout)

	def execute_set_object_property(self, object:str, variable:str, value:str, timeout:float=5):
		"""Set object property. \n\tUsage: SetObjectProperty [path/alias/objectname] [variable] [value]"""
		if not (isinstance(object, str)):
			print("set_object_property: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(variable, str)):
			print("set_object_property: variable has wrong type. Expected str got {}".format(variable.__class__.__name__))
		if not (isinstance(value, str)):
			print("set_object_property: value has wrong type. Expected str got {}".format(value.__class__.__name__))
		self.execute("SetObjectProperty {} {} {}".format(object, variable, value), timeout)

	def request_set_object_property(self, object:str, variable:str, value:str, timeout:float=5):
		"""Set object property. \n\tUsage: SetObjectProperty [path/alias/objectname] [variable] [value]"""
		if not (isinstance(object, str)):
			print("set_object_property: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(variable, str)):
		...
		if not (isinstance(value, str)):
			print("set_object_property: value has wrong type. Expected str got {}".format(value.__class__.__name__))
		return self.request("SetObjectProperty {} {} {}".format(object, variable, value), timeout)

	def execute_get_object_property(self, object:str, variable:str, timeout:float=5):
		"""Get object property. \n\tUsage: GetObjectProperty [path/alias/objectname] [variable]"""
		if not (isinstance(object, str)):
			print("get_object_property: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(variable, str)):
			print("get_object_property: variable has wrong type. Expected str got {}".format(variable.__class__.__name__))
		self.execute("GetObjectProperty {} {}".format(object, variable), timeout)

	def request_get_object_property(self, object:str, variable:str, timeout:float=5):
		"""Get object property. \n\tUsage: GetObjectProperty [path/alias/objectname] [variable]"""
		if not (isinstance(object, str)):
			print("get_object_property: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(variable, str)):
	...
	def execute_change_sensor_profile(self, object:str, filename:str, timeout:float=5):
		"""Change sensor profile for a vehicle. \n\tUsage: ChangeSensorProfile [path/alias/objectname] [filename]"""
		if not (isinstance(object, str)):
			print("change_sensor_profile: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(filename, str)):
			print("change_sensor_profile: filename has wrong type. Expected str got {}".format(filename.__class__.__name__))
		self.execute("ChangeSensorProfile {} {}".format(object, filename), timeout)

	def request_change_sensor_profile(self, object:str, filename:str, timeout:float=5):
		"""Change sensor profile for a vehicle. \n\tUsage: ChangeSensorProfile [path/alias/objectname] [filename]"""
		if not (isinstance(object, str)):
			print("change_sensor_profile: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(filename, str)):
			print("change_sensor_profile: filename has wrong type. Expected str got {}".format(filename.__class__.__name__))
		return self.request("ChangeSensorProfile {} {}".format(object, filename), timeout)

	def execute_get_object_location(self, object:str, timeout:float=5):
		"""Get the location of an object. \n\tUsage: GetObjectLocation [path/alias/objectname]"""
		if not (isinstance(object, str)):
			print("get_object_location: object has wrong type. Expected str got {}".format(object.__class__.__name__))
	...
	def execute_get_object_rotation(self, object:str, timeout:float=5):
		...
		if not (isinstance(object, str)):
			print("get_object_rotation: object has wrong type. Expected str got {}".format(object.__class__.__name__))
	...
	def execute_get_camera_fov(self, ego_object:str, camera_path:str, timeout:float=5):
		...
		if not (isinstance(ego_object, str)):
			print("get_camera_fov: ego_object has wrong type. Expected str got {}".format(ego_object.__class__.__name__))
		if not (isinstance(camera_path, str)):
			print("get_camera_fov: camera_path has wrong type. Expected str got {}".format(camera_path.__class__.__name__))
		self.execute("GetCameraFoV {} {}".format(ego_object, camera_path), timeout)

	def request_get_camera_fov(self, ego_object:str, camera_path:str, timeout:float=5):
		"""Get the vertical and horizontal FoV of the camera sensor. \n\tUsage: GetCameraFoV [Ego path/alias/objectname] [camera path]"""
		if not (isinstance(ego_object, str)):
			print("get_camera_fov: ego_object has wrong type. Expected str got {}".format(ego_object.__class__.__name__))
		if not (isinstance(camera_path, str)):
			print("get_camera_fov: camera_path has wrong type. Expected str got {}".format(camera_path.__class__.__name__))
		return self.request("GetCameraFoV {} {}".format(ego_object, camera_path), timeout)

	def request_set_ptz_offset(self, object:str, camera_path:str, pan:str, tilt:str, zoom:str, timeout:float=5):
		"""Set the pan, tilt and zoom of a camera. \n\tUsage: SetPTZOffset [Ego path/alias/objectname] [camera path] [pan] [tilt] [zoom]"""
		if not (isinstance(object, str)):
			print("set_ptz_offset: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(camera_path, str)):
			print("set_ptz_offset: camera_path has wrong type. Expected str got {}".format(camera_path.__class__.__name__))
		if not (isinstance(pan, str)):
			print("set_ptz_offset: pan has wrong type. Expected str got {}".format(pan.__class__.__name__))
		if not (isinstance(tilt, str)):
			print("set_ptz_offset: tilt has wrong type. Expected str got {}".format(tilt.__class__.__name__))
		if not (isinstance(zoom, str)):
			print("set_ptz_offset: zoom has wrong type. Expected str got {}".format(zoom.__class__.__name__))
		return self.request("SetPTZOffset {} {} {} {} {}".format(object, camera_path, pan, tilt, zoom), timeout)

	def execute_get_camera_distortion_params(self, object:str, camera_path:str, timeout:float=5):
		...
		if not (isinstance(object, str)):
			print("get_camera_distortion_params: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(camera_path, str)):
			print("get_camera_distortion_params: camera_path has wrong type. Expected str got {}".format(camera_path.__class__.__name__))
		self.execute("GetCameraDistortionParams {} {}".format(object, camera_path), timeout)

	def request_get_camera_distortion_params(self, object:str, camera_path:str, timeout:float=5):
		"""Get the distortion parameters of the camera sensor. They are returned in the order: k1,k2,p1,p2,k3 \n\tUsage: GetCameraDistortionParams [Ego path/alias/objectname] [camera path]"""
		if not (isinstance(object, str)):
			print("get_camera_distortion_params: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(camera_path, str)):
			print("get_camera_distortion_params: camera_path has wrong type. Expected str got {}".format(camera_path.__class__.__name__))
		return self.request("GetCameraDistortionParams {} {}".format(object, camera_path), timeout)
	...
	def execute_get_lidar_info(self, object:str, camera_path:str, timeout:float=5):
		...
		if not (isinstance(object, str)):
			print("get_lidar_info: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(camera_path, str)):
			print("get_lidar_info: camera_path has wrong type. Expected str got {}".format(camera_path.__class__.__name__))
	...
	def execute_get_center_of_mass(self, object:str, timeout:float=5):
		...
		if not (isinstance(object, str)):
			print("get_center_of_mass: object has wrong type. Expected str got {}".format(object.__class__.__name__))
	...
	def execute_get_sensor_distribution_info(self, timeout:float=5):
		...
		self.execute("GetSensorDistributionInfo", timeout)

	def request_get_sensor_distribution_info(self, timeout:float=5):
		"""Get list of sensor information. Data returned in format: 'Client ID;Sensor Path;Ego Filename;Ego Path;Ego Alias;Sensor Type;Sensor Port;Sensor IP;' and groups with the format 'ClientID;GroupName;SocketPort;SocketIP;SocketPRotocol;StreamToNetwork'"""
		return self.request("GetSensorDistributionInfo", timeout)

	def execute_get_sensor_info(self, object:str, sensor_path:str, timeout:float=5):
		"""Returns the currently set parameters for a specific sensor. \n\tUsage: GetSensorInfo [Ego path/alias/objectname] [sensor path]"""
		if not (isinstance(object, str)):
			print("get_sensor_info: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(sensor_path, str)):
			print("get_sensor_info: sensor_path has wrong type. Expected str got {}".format(sensor_path.__class__.__name__))
		self.execute("GetSensorInfo {} {}".format(object, sensor_path), timeout)

	def request_get_sensor_info(self, object:str, sensor_path:str, timeout:float=5):
		"""Returns the currently set parameters for a specific sensor. \n\tUsage: GetSensorInfo [Ego path/alias/objectname] [sensor path]"""
		if not (isinstance(object, str)):
			print("get_sensor_info: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(sensor_path, str)):
	...
	def execute_set_object_velocity(self, object:str, local_space:bool, X:float, Y:float, Z:float, timeout:float=5):
		"""Set object velocity in centimeters per second. \n\tUsage: SetObjectVelocity [path/alias/objectname] [local space] [X] [Y] [Z]"""
		if not (isinstance(object, str)):
			print("set_object_velocity: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(local_space, bool)):
			print("set_object_velocity: local_space has wrong type. Expected bool got {}".format(local_space.__class__.__name__))
		if not (isinstance(X, int) or isinstance(X, float)):
			print("set_object_velocity: X has wrong type. Expected float got {}".format(X.__class__.__name__))
		if not (isinstance(Y, int) or isinstance(Y, float)):
			print("set_object_velocity: Y has wrong type. Expected float got {}".format(Y.__class__.__name__))
		if not (isinstance(Z, int) or isinstance(Z, float)):
			print("set_object_velocity: Z has wrong type. Expected float got {}".format(Z.__class__.__name__))
		self.execute("SetObjectVelocity {} {} {} {} {}".format(object, local_space, X, Y, Z), timeout)

	def request_set_object_velocity(self, object:str, local_space:bool, X:float, Y:float, Z:float, timeout:float=5):
		"""Set object velocity in centimeters per second. \n\tUsage: SetObjectVelocity [path/alias/objectname] [local space] [X] [Y] [Z]"""
		if not (isinstance(object, str)):
			print("set_object_velocity: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(local_space, bool)):
		...
		if not (isinstance(Z, int) or isinstance(Z, float)):
			print("set_object_velocity: Z has wrong type. Expected float got {}".format(Z.__class__.__name__))
		return self.request("SetObjectVelocity {} {} {} {} {}".format(object, local_space, X, Y, Z), timeout)

	def execute_set_object_angular_velocity(self, object:str, local_space:bool, X:float, Y:float, Z:float, timeout:float=5):
		"""Set object angular velocity in degrees per second. \n\tUsage: SetObjectAngularVelocity [path/alias/objectname] [local space] [X] [Y] [Z]"""
		if not (isinstance(object, str)):
			print("set_object_angular_velocity: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(local_space, bool)):
			print("set_object_angular_velocity: local_space has wrong type. Expected bool got {}".format(local_space.__class__.__name__))
		if not (isinstance(X, int) or isinstance(X, float)):
			print("set_object_angular_velocity: X has wrong type. Expected float got {}".format(X.__class__.__name__))
		if not (isinstance(Y, int) or isinstance(Y, float)):
			print("set_object_angular_velocity: Y has wrong type. Expected float got {}".format(Y.__class__.__name__))
		if not (isinstance(Z, int) or isinstance(Z, float)):
			print("set_object_angular_velocity: Z has wrong type. Expected float got {}".format(Z.__class__.__name__))
		self.execute("SetObjectAngularVelocity {} {} {} {} {}".format(object, local_space, X, Y, Z), timeout)

	def request_set_object_angular_velocity(self, object:str, local_space:bool, X:float, Y:float, Z:float, timeout:float=5):
		"""Set object angular velocity in degrees per second. \n\tUsage: SetObjectAngularVelocity [path/alias/objectname] [local space] [X] [Y] [Z]"""
		if not (isinstance(object, str)):
			print("set_object_angular_velocity: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(local_space, bool)):
		...
		if not (isinstance(Z, int) or isinstance(Z, float)):
			print("set_object_angular_velocity: Z has wrong type. Expected float got {}".format(Z.__class__.__name__))
		return self.request("SetObjectAngularVelocity {} {} {} {} {}".format(object, local_space, X, Y, Z), timeout)
	...
	def execute_trigger_object(self, object:str, TriggerName:str, timeout:float=5):
		...
		if not (isinstance(object, str)):
			print("trigger_object: object has wrong type. Expected str got {}".format(object.__class__.__name__))
		if not (isinstance(TriggerName, str)):
			print("trigger_object: TriggerName has wrong type. Expected str got {}".format(TriggerName.__class__.__name__))
	...
	def execute_gps_to_cartesian(self, latitude:float, longitude:float, timeout:float=5):
		...
		if not (isinstance(latitude, int) or isinstance(latitude, float)):
			print("gps_to_cartesian: latitude has wrong type. Expected float got {}".format(latitude.__class__.__name__))
		if not (isinstance(longitude, int) or isinstance(longitude, float)):
			print("gps_to_cartesian: longitude has wrong type. Expected float got {}".format(longitude.__class__.__name__))
		self.execute("GPSToCartesian {} {}".format(latitude, longitude), timeout)

	def request_gps_to_cartesian(self, latitude:float, longitude:float, timeout:float=5):
		"""Converts the specified geographic coordinates to Cartesian coordinates (x, y). \n\tUsage: GPSToCartesian [latitude] [longitude]"""
		if not (isinstance(latitude, int) or isinstance(latitude, float)):
			print("gps_to_cartesian: latitude has wrong type. Expected float got {}".format(latitude.__class__.__name__))
		if not (isinstance(longitude, int) or isinstance(longitude, float)):
	...
	def execute_get_all_object_properties(self, object:str, timeout:float=5):
		...
		if not (isinstance(object, str)):
			print("get_all_object_properties: object has wrong type. Expected str got {}".format(object.__class__.__name__))
