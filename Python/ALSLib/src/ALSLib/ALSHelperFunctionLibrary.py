...
def RestoreFromOverrideFile(ConfigFilePath, OverrideFilePath):
	...
	ListOfFiles = list(set(map(lambda x: os.path.join(ConfigFilePath, x[0]), overrides)))
...
def GenerateConfigsFromOverrides(ConfigFilePath, OverrideFilePath):
	...
	GenerateFilesForConfig(list_of_files, list_of_variable_path_lists, list_of_value_lists)
...
def build_override_array(file_name, array_path, array_entries):
	return build_override(file_name, array_path, build_override_array_value(array_entries))
...
def build_override_vector(file_name, vector_path, x, y, z, w = None):
	return build_override(file_name, vector_path, build_override_vector_value(x, y, z, w))
...
def build_override_transform_value(translation, rotation, scale):
	...
	value = "{\n(\n"
	if (len(rotation) > 3):
		value += "Rotation=" + build_override_vector_value(rotation[0], rotation[1], rotation[2], rotation[3])
		value += "Translation=" + build_override_vector_value(translation[0], translation[1], translation[2])
		value += "Scale3D=" + build_override_vector_value(scale[0], scale[1], scale[2])
	elif (len(rotation) == 3):
		# Euler values only (PositionEuler)
		value += "Location=" + build_override_vector_value(translation[0], translation[1], translation[2])
		value += "Rotation=" + build_override_vector_value(rotation[0], rotation[1], rotation[2])
...
def build_override_transform(file_name, vector_path, location, rotation, scale):
	return build_override(file_name, vector_path, build_override_transform_value(location, rotation, scale))
...
def ReadUint8(buffer, index):
	return (int.from_bytes(struct.unpack('<c', buffer[index:index+1])[0], byteorder='big', signed=False), index + 1)
...
def ReadImage_Stream(buffer, index):
	...
	index 	   += image_size
	img 	 	= ALSImg.DecodeImageData(image_data, image_width, image_height, image_channels)
	return img, index, image_width, image_height

def ReadImage_Group(buffer, index):
	image_width, 	index	= ReadUint32(buffer, index)
	image_height, 	index	= ReadUint32(buffer, index)
	image_channels, index 	= ReadUint8(buffer, index)
	image_format, index 	= ReadUint8(buffer, index)
	image = None
	if image_format == 1:
		...
		image_data = buffer[index:index+image_size]
		index += image_size
		image = ALSImg.DecodeImageData(image_data, image_width, image_height, image_channels)
	return image, index, image_width, image_height
...
def find_install_path(require_valid_path:bool=True):
	...
	while (not install_path) and (os.path.split(cd)[1] != ""):
		...
		if ("AILiveSim.exe" in files) or ("ConfigFiles" in folders and "Config" in folders):
			install_path = cd
		else:
	...
	if require_valid_path and not install_path:
		raise FileNotFoundError("Could not find the installation path. Is the script running in the correct directory?")
...
def get_root_folder_path(folder_name:str, add_on_path:str=None):
	...
	if add_on_path:
		final_path = os.path.normpath(os.path.join(final_path, add_on_path.strip("/").strip("\\")))
	return final_path
