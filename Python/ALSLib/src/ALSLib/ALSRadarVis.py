...
class MainWindow(QMainWindow):
	def __init__(self, save_images=True, 
		...
		self.radar = Sensor(self.sensor_sizex, self.sensor_sizey, default_range, parent=self)
		...
		self.range_slider, self.range_slide_val = self.add_slider("Range:", self.zoom_slider_value_changed, min_range, max_range, default_range, 'nmi', 5)
		self.gain_slider, self.gain_slider_val = self.add_slider("Gain:", self.gain_slider_value_changed, min_gain, max_gain, default_gain)
		self.size_slider, self.size_slider_val = self.add_slider("Size:", self.pointsize_slider_value_changed, min_pointsize, max_pointsize, self.radar.size_multiplier, 'px')
		self.lifetime_slider, self.lifetime_slider_val = self.add_slider("LifeTime:", self.lifetime_slider_value_changed, min_lifetime, max_lifetime, default_lifetime, 'frames')
		self.ringdist_slider, self.ringdist_slider_val = self.add_slider("RingDist:", self.ringrange_slider_value_changed, 0.1, 10, 1, 'nmi')
	...
	def add_slider(self, title, func, rangemin, rangemax, default_val, unit = '', range_step = 1):
	...
	def save_radar_image(self):
		if self.save_images and self.image_need_save:
			...
			filename = 'img' + str(self.image_ID) + '_' + str(self.image_timestamp) + '_' + str(self.image_sensor_pos) + '.bmp'
...
class Dot():
	...
	def draw(self):
		...
		glColor4f(self.red, self.green, self.blue, self.alpha * (self.timer / self.total_lifetime))
...
class Sensor(QOpenGLWidget):
	...
	def resizeGL(self, w, h):
		if w > 0 and h > 0:
			...
			glOrtho(-scale_multiplier * aspect / 2, 
...
class RadarVisualization:
	...
	def add_dot(self, dot:Dot):
		...
		dot.set_rgba(dot.red, dot.green, dot.blue, 0.1 * self.window.radar.gain_multiplier)
		dot.setSize(dot.w * self.window.radar.size_multiplier, dot.h * self.window.radar.size_multiplier)
