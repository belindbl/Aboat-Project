...
def get_lidar_header_angle_distance(data):
    ...
    return posX,posY,posZ,quatW,quatX,quatY,quatZ,num_cols,num_beams_per_col,col_time,FrameID,\
    ColId, pointcloud_data
...
def get_lidar_header(data):
    ...
    return posX,posY,posZ,quatW,quatX,quatY,quatZ,numPoints,timeStart,timeEnd,\
    numberOfBeams, pointCloudData
...
def get_lidar_header_from_group(data, index = 0):
    ...
    return posX,posY,posZ,quatW,quatX,quatY,quatZ,numPoints,timeStart,timeEnd,\
    numberOfBeams, pointCloudData, index

def get_lidar_header_livox(data):
    sizeofFloat = 4
    index = 9
    posX,posY,posZ,quatW,quatX,quatY,quatZ,timeStart,\
    numPoints = struct.unpack('<fffffffff', data[0:index*sizeofFloat])
    pointCloudData = data[index*sizeofFloat:]
    return posX,posY,posZ,quatW,quatX,quatY,quatZ,timeStart,\
    numPoints, pointCloudData

def read_livox_data(data, num_points):
    readPoints = 0
    sizeofuint8 = 1
    sizeofFloat = 4
    ...
    while readPoints < num_points:
        ...
        offsetTime,pointX,pointY,pointZ,intensity,tag,number = struct.unpack('fffffBB', data[start:end])#ReadLivoxPoint()
    ...
    for file in files:
        if os.path.isdir(os.path.join(basepath, file)) and file == "SensorData":
            ...
            for inner_file in inner_files:
                if os.path.isdir(os.path.join(sensordata_path, inner_file)) and inner_file == "pcl":
                    has_pcl = True
    if not has_data:
        os.mkdir(sensordata_path)
    if not has_pcl:
        os.mkdir(os.path.join(sensordata_path, "/pcl"))
