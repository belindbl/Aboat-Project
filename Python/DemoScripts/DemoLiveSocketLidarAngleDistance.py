...
###############################################################################
# This demo shows how to collect the lazer proximity data
#
# we use a premade sensorprofile where all the sensors are pre-configured, but 
# disabled so we use overrrides to only enable the sensor we want in this case
###############################################################################
...
if __name__ == "__main__":
	...
	while(True):
		...
		h_steps = 360/1024 #this is from config files o fScannerAngleDistance
		v_steps = -30/num_beams_per_col #this is from config files o fScannerAngleDistance
		v_span_degree = 30
		...
		for count, d in enumerate(readings_array):
			...
			reading = reading_to_3d(d, 90-(v_steps*(count-num_beams_per_col*0.5)), 180-h_steps*ColId, readings_array[count+num_beams_per_col] )
