...
# NOTE Working 2025-01-28, captures both LIDAR and camera data, 
# however, the camera is around 57 samples behind the LIDAR

# Global variables
HOST = '127.0.0.1' # Host
ABOAT = "EgoVehicleSettings\Aboat.SensorProfile.ini" # Sensor profile, CAM = 0, LIDAR = 1
LIDAR = "ConfigFiles\Sensors\LidarSettings\LIDARTest.xml" # Sensor config for the LIDAR
...
def lidar_thread():
    ...
    while lidar_pcd_count < MAX_LIDAR_PCDS and not stop_threads_event.is_set():  # Stop
        # Read LiDAR data
        data = client.read()
        sizeofFloat = 4
        index = 11
...
if __name__ == "__main__":
    ...
    #TestContext.client.request_load_situation_layer("DemoSensorsTestAboat")
